...
class Bubble(pygame.sprite.Sprite):
    def __init__(self, image, color, position=(0, 0), row_idx=-1, col_idx=-1):
        ...
        self.radius = 18  # 값이 커질수록 공이 빨리 움직인다
...
def process_collision():
    ...
    if hit_bubble or curr_bubble.rect.top <= wall_height:
        row_idx, col_idx = get_map_index(*curr_bubble.rect.center)  # * : unpacking
...
def visit(row_idx, col_idx, color=None):
    # map의 범위를 벗어나는지 학인
    if row_idx < 0 or row_idx >= MAP_ROW_COUNT or col_idx < 0 or row_idx >= MAP_COLUMN_COUNT:
        return
    # 현재 셀의 색상이 color와 같은지 롹인
    if color and map[row_idx][col_idx] != color:
        return
    # 버블이 빈 공간이거나 버블이 존재할 수 없는 위치인지 학인
...
def remove_visited_bubbles():
    ...
    for bubble in bubbles_to_remove:
        map[bubble.row_idx][bubble.col_idx] = "."  # 비어 있음을 표시
...
# 배경 이미지 불러오기
current_path = os.path.dirname(__file__)  # 지금 실행하는 화일 경로
background = pygame.image.load(os.path.join(current_path, "background.png"))
# 벽 이미지 불러오기
wall = pygame.image.load(os.path.join(current_path, "wall.png"))

# 버블 이밎 불러오기
# google : 온라인 포토샵 : pixir.com
bubble_images = [
...
map = []  # map
visited = []  # visited postion
bubble_group = pygame.sprite.Group()
setup()

running = True
while running:
    clock.tick(60)  # FPS 60으로설정
