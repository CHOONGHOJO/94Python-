...
def setup_gemstone():
    ...
    gemstone_group.add(
        Gemstone(gemstone_images[0], (400, 400), small_gold_price, small_gold_speed))
    gemstone_group.add(
        Gemstone(gemstone_images[0], (600, 450), small_gold_price, small_gold_speed))
    gemstone_group.add(
        Gemstone(gemstone_images[0], (800, 400), small_gold_price, small_gold_speed))
    gemstone_group.add(
        Gemstone(gemstone_images[0], (1150, 380), small_gold_price, small_gold_speed))
    # 큰 금circle(900, 420)))
    gemstone_group.add(Gemstone(gemstone_images[1], (300, 500), big_gold_price, big_gold_speed))
    gemstone_group.add(
        Gemstone(gemstone_images[1], (800, 500), big_gold_price, big_gold_speed))
    # 돌
    gemstone_group.add(
    ...
    gemstone_group.add(
        Gemstone(gemstone_images[3], (150, 500), diamond_price, diamond_speed))

def update_socre(score):
    global curr_score
    curr_score += score
...
def display_score():
    ...
    screen.blit(txt_curr_score, (50, 80))


def display_time(time):
    txt_timer = game_font.render(f"Time : {time}", True, BLACK)
    screen.blit(txt_timer, (1150, 20))


def display_game_over():
    game_font = pygame.font.SysFont("arialrounded", 60)  # 60 크기
    txt_game_over = game_font.render(game_result, True, BLACK)
    rect_game_over = txt_game_over.get_rect(
        center=(int(screen_width/2), int(screen_height/2)))  # 화면 중앙 표시
...
while running:
    ...
    if claw.offset.x < default_offset_x_claw:  # 원위치에 오면
        ...
        if caught_gemstone:  # 잡힌 보석이 있다면
            update_socre(caught_gemstone.price)  # 점수 업데이트 처리
    ...
    if not caught_gemstone:  # 잡힌 보석이 없다면 충돌 체크
        for gemstone in gemstone_group:
            ...
            # 투명 영역 제외, 실제 이미지 존재 부분에 대해 충돌 처리
...
pygame.time.delay(3000)  # 2초 대기후 종료
pygame.quit()
